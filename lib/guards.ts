import type { CrewFirstCallsheet, CrewFirstLocation } from '../services/extractor-universal/config/schema';
...
export function isCallsheetSimple(x: any): x is { date: string; projectName: string; productionCompanies: string[]; locations: string[] } {
  return (
    x && typeof x === 'object' &&
...
// Full crew-first callsheet with structured locations
export function isCrewFirstCallsheet(x: any): x is CrewFirstCallsheet {
  // First check if it's the new crew-first format
  if (x && typeof x === 'object' && x.version === 'parser-crew-1') {
    return (
      typeof x.date === 'string' &&
      typeof x.projectName === 'string' &&
  ...
  // Fall back to simple format for backward compatibility
  return isCallsheetSimple(x);
}
