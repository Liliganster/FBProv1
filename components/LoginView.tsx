  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      try {
        ...
        setInfo(`${t('login_verify_notice_title')}: ${t('login_verify_notice_body')}`);
        setIsLogin(true);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
    ...
    <div className="flex items-center justify-center min-h-screen relative overflow-hidden">
      ...
      <div className="w-full max-w-md p-8 space-y-8 bg-frost-glass backdrop-blur-glass rounded-organic shadow-glass border-glass relative z-20">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          {info && <p className="text-sm text-green-300 text-center bg-green-900/20 border border-green-500/20 rounded-smooth px-3 py-2">{info}</p>}
